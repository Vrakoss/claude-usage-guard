...
- Tests: numbered files (`t1`–`t13`) with one concern each; build fakes via `makeDeps()` from `test/helpers.mjs` (in-memory fs recorder, fixed clock `FIXED_NOW_MS`, pid 4242, recorded fetch/exec/exit calls). Exception: `t12-entrypoint.mjs` spawns the real script (entry-point detection isn't reachable through `main(deps)`). Time-formatting assertions must be timezone-safe: assert shape/month (`Nov`, `\d{2}:\d{2}`), never an exact weekday or day-of-month — CI runs in multiple timezones' default (UTC) but local runs don't.
