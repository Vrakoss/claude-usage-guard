...
- **Data flow:** stdin JSON → `parseHookInput` (defaults to `UserPromptSubmit` on any garbage) → `acquireData` (fresh cache within TTL → use; recent failure → 5-min negative cache; else read token and fetch) → `parseWindows(data, now)` (drops windows whose reset already passed — stale data must never block) → `evaluateThresholds` → output.
...
- Tests: numbered files (`t1`–`t11`) with one concern each; build fakes via `makeDeps()` from `test/helpers.mjs` (in-memory fs recorder, fixed clock `FIXED_NOW_MS`, pid 4242, recorded fetch/exec/exit calls). Time-formatting assertions must be timezone-safe: assert shape/month (`Nov`, `\d{2}:\d{2}`), never an exact weekday or day-of-month — CI runs in multiple timezones' default (UTC) but local runs don't.
